...
> extends BaseAPI {
  ...
  async update(data: TUpdateInput | FormData, options?: MutationOptions): Promise<TBase> {
    ...
    generateUsersPermissionsUserAPI() {
...
> extends CollectionAPI<TBase, TCreateInput, TUpdateInput, TFilters, TPopulateKeys> {
  ...
  protected unwrap(response: any): any {
    ...
    generateCustomAPIClasses(schema, parsedRoutes) {
        const classes = []
        for (const [controller, routes] of parsedRoutes.byController) {
            ...
            if (contentType) {
                ...
                classes.push(`// Custom API class for ${contentType.cleanName} (${contentType.kind} type) with custom routes
...
${customMethods}
                ...
                classes.push(`// Standalone API class for ${controller} controller
...
${customMethods}
     ...
     */
    resolveContentTypeApiClass(contentType, hasCustomRoutes) {
    ...
    generateStrapiClient(
        schema,
        parsedRoutes,
        activePlugins = PLUGIN_REGISTRY,
    ) {
...
${standaloneInits}
  ...
  async validateSchema(): Promise<{
    ...
    buildStandaloneDeclarations(schema, parsedRoutes) {
    ...
    buildStandaloneInits(schema, parsedRoutes) {
        if (!parsedRoutes) return ''
        const inits = []
        for (const [controller, routes] of parsedRoutes.byController) {
            if (controller === 'auth' || controller === 'user') continue
